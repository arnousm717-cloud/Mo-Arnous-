...
import { handleGetNote, handleUpdateNote, handleDeleteNote } from "./handlers";
...
export const GET = withRequestLogging(
  ...
  async (_request: NextRequest, { params }: { params: Promise<{ id: string }> }) => {
    ...
    return handleGetNote(user?.id ?? null, id);
  },
);
...
export const PATCH = withRequestLogging(
  ...
  async (request: NextRequest, { params }: { params: Promise<{ id: string }> }) => {
    ...
    return handleUpdateNote(user?.id ?? null, id, body, request.headers.get("Idempotency-Key"));
  },
);
...
export const DELETE = withRequestLogging(
  ...
  async (request: NextRequest, { params }: { params: Promise<{ id: string }> }) => {
    ...
    const { id } = await params;
    const user = await getAuthenticatedUser();
    return handleDeleteNote(user?.id ?? null, id);
  },
);
