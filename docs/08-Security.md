...
- **Staff/product users**: Supabase Auth (email/password + OAuth providers as configured), session JWTs short-lived with refresh rotation. MFA (TOTP) available at the organization level, required for `agency_owner`/`agency_admin` roles once available (Phase 8 enterprise hardening; not an MVP blocker per the SOC2 deferral).
...
- **Runbooks**: documented recovery steps for the most likely failure modes (accidental tenant data deletion via `data_subject_requests` misconfiguration, n8n instance crash, provider API outage cascading into stuck `workflow_runs`) — maintained in `docs/runbooks/` as they're written, not left as tribal knowledge.
