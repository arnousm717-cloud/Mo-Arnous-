...
| `intelligence` | Visitor identification logic, enrichment orchestration, lead-scoring engine (rules-based + agent-assisted blending). **Implemented as of Milestone 3.1B**, initial scope only: `checkCookieTrackingConsent`/`resolveOrCreateVisitor`/`resolveOrCreateVisitorSession`/`appendVisitorEvent`/`ingestTrackingEvent` — atomic anonymous-event ingestion, consent-gated. Not yet implemented: visitor identification (matching to a real `contacts` row — Milestone 3.2), enrichment orchestration, lead scoring. Actual current dependency is `database` only, narrower than the row below — `crm` was never added, since 3.1B performs no identification/business logic and this package follows the same "don't take on a dependency for functionality that doesn't exist yet" discipline already established for `crm`/`compliance`/`tenancy`'s own real `package.json` (each lists `database` only despite this table's own broader description). The `crm` dependency below remains this package's documented eventual scope, for Milestone 3.2's identification work, once it actually needs it | `database`, `crm` |
