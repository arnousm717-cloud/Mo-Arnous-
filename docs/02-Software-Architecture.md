...
| `intelligence` | Visitor identification logic, enrichment write-back, lead-scoring engine (rules-based + agent-assisted blending). **Implemented as of Milestone 3.1B**, initial scope: `checkCookieTrackingConsent`/`resolveOrCreateVisitor`/`resolveOrCreateVisitorSession`/`appendVisitorEvent`/`ingestTrackingEvent` — atomic anonymous-event ingestion, consent-gated. **Visitor identification (matching an anonymous visitor to a real `contacts` row) is implemented as of Milestone 3.2** — `identifyVisitor` (`packages/intelligence/src/identify.ts`), the one atomic transaction behind `POST /track/identify`. **Enrichment write-back is implemented as of Milestone 3.3** — `recordEnrichmentResult`/`recordWorkflowRunStarted`/`recordWorkflowRunTriggerFailed` (`packages/intelligence/src/enrichment.ts`), the provider-agnostic domain layer behind the two API-key-authenticated write-back endpoints (`04-API-Architecture.md` §2.8); this package never calls a provider or holds a provider credential — only records an already-normalized result n8n pushes back. Lead scoring remains not yet implemented. Actual current dependency is `database` and `crm` — `crm` was added in Milestone 3.2C specifically for `getContactByEmail`, the deterministic contact-resolution step `identifyVisitor` needs; unlike `lead scoring` above, this is no longer eventual scope | `database`, `crm` |
