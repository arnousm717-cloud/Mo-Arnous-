...
| `brain` | AI Revenue Brain: ingestion orchestration, structured entity profiles, vector embeddings, retrieval client (`11-AI-Revenue-Brain.md`). **CRM domain-event ingestion (deterministic canonical-profile projection) implemented as of Milestone 4.1 Phase 2** — see the implementation note below. Actual current dependency is `database` and `crm` — this package never calls `auth` directly, since tenant context is always already resolved by its callers | `database`, `crm` |
