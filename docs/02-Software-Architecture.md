...
| `crm` | Companies/contacts/deals/pipeline business rules — stage transitions, deal validation, activity logic. **Read-only dashboard aggregates implemented as of Milestone 3.5** — `getDealDashboardMetrics` (`packages/crm/src/dashboard-metrics.ts`): organization-wide, currency-segregated deal KPIs (open count, open pipeline value, win rate, average open deal size, deals-by-stage), each a single SQL aggregate query, never a fetch-everything-then-sum-in-Node pattern. `listDeals` (unmodified since Milestone 2.2) is reused as-is for the dashboard's "Recently Created Deals" section — no new query was added for it | `database`, `auth` |
| `intelligence` | Visitor identification logic, enrichment write-back, deterministic rules-based lead scoring. **Implemented as of Milestone 3.1B**, initial scope: `checkCookieTrackingConsent`/`resolveOrCreateVisitor`/`resolveOrCreateVisitorSession`/`appendVisitorEvent`/`ingestTrackingEvent` — atomic anonymous-event ingestion, consent-gated. **Visitor identification (matching an anonymous visitor to a real `contacts` row) is implemented as of Milestone 3.2** — `identifyVisitor` (`packages/intelligence/src/identify.ts`), the one atomic transaction behind `POST /track/identify`. **Enrichment write-back is implemented as of Milestone 3.3** — `recordEnrichmentResult`/`recordWorkflowRunStarted`/`recordWorkflowRunTriggerFailed` (`packages/intelligence/src/enrichment.ts`), the provider-agnostic domain layer behind the two API-key-authenticated write-back endpoints (`04-API-Architecture.md` §2.8); this package never calls a provider or holds a provider credential — only records an already-normalized result n8n pushes back. **Lead scoring is implemented as of Milestone 3.4** — `computeScore`/`recalculateContactScore`/`recalculateContactScoreForEvent`/`recoverPendingPostEnrichmentScoring` (`packages/intelligence/src/scoring.ts`); deterministic and rules-based only, no AI/ML/agent involvement of any kind — see `08-Security.md` §2.1. **Read-only dashboard aggregates implemented as of Milestone 3.5** — `getLeadScoreDistribution`/`getHighScoreContacts`/`getIdentifiedVisitorMetrics` (`packages/intelligence/src/dashboard-metrics.ts`): the first two select only each contact's latest historized `lead_scores` row (`DISTINCT ON`, never a naive count over full history); the third counts currently-identified visitors whose most recent `visitor_identifications` event occurred within a fixed 30-day window — `website_visitors.first_seen_at` is deliberately never used as an identification timestamp. Actual current dependency is `database` and `crm` — `crm` was added in Milestone 3.2C specifically for `getContactByEmail`, the deterministic contact-resolution step `identifyVisitor` needs | `database`, `crm` |
...
| `brain` | AI Revenue Brain: ingestion orchestration, structured entity profiles, vector embeddings, retrieval client (`11-AI-Revenue-Brain.md`). **CRM domain-event ingestion (deterministic canonical-profile projection) implemented as of Milestone 4.1 Phase 2**; **embedding write-back (persisting an externally-computed vector, never generating one) implemented as of Milestone 4.1 Phase 3** — see the implementation notes below. Actual current dependency is `database` and `crm` — this package never calls `auth` directly, since tenant context is always already resolved by its callers | `database`, `crm` |
...
**M1.7 implementation note**: the `(event_id, consumer)` idempotency this section already prescribed is realized as a real table, `event_deliveries` (`03-Database-Architecture.md` §2.10) — `events.processed_at` alone cannot express it once more than one consumer exists, since it's a single global flag, not a per-consumer one. Dispatch in M1.7 was an **in-process TS function** (`packages/database`'s `dispatchPendingEvents()`), called directly rather than by any scheduled mechanism — n8n fan-out was still Phase 3.

**Milestone 3.3 implementation note (delivered) — dispatch is now real, cron-driven, and reliability-hardened after two hostile-audit-driven remediation rounds**: `GET /api/internal/dispatch-events` (Vercel Cron, every minute, `CRON_SECRET`-protected, `04-API-Architecture.md` §2.8) is the real scheduled trigger this section originally anticipated — realized as a Vercel Cron-invoked API route, not a Supabase Edge Function (none exists in this codebase, `08-Security.md` §7.1). Each invocation reads at most a fixed `DISPATCH_BATCH_SIZE` (10) pending events, never an unbounded pass, and no database transaction of any kind spans an external HTTP call. `event_deliveries` gained `status`/`lease_expires_at` (`03-Database-Architecture.md` §2.10) specifically to close a real crash window a first remediation round's own "claim-first" design left open — a process death between claiming a delivery and completing it previously left a row permanently indistinguishable from a genuine success, with no automated recovery; reproduced directly against real Postgres before the fix, closed by a `leased`/`delivered` lease model with a fixed 120-second expiry, requiring no separate sweeper process. At-least-once delivery, not exactly-once, remains the real, disclosed contract: any consumer with a non-transactional external effect (an API call) must be idempotent on its own terms — this dispatcher's own bookkeeping cannot make an external side effect exactly-once, only guarantee its own delivery records never duplicate.

**Milestone 3.4 implementation note (delivered) — the "recalculating a lead score when a new `contact_enrichment` row lands" example this section already named is now real, on two independent paths**: a second `EventConsumer` (`lead_scoring`) is registered on the same `visitor.identified` event type the dispatcher above already drains, reusing `workflow_runs` for its own event-trigger deduplication (an explicit atomic claim, the same pattern proven for `event_deliveries`' own lease) rather than a new mechanism. Separately, a successful enrichment write-back itself triggers a recalculation under its own distinct `workflow_runs` key, with a durable, automatically-retried recovery path (`recoverPendingPostEnrichmentScoring`, invoked by the same cron tick as the dispatcher above) for a failed or crashed-mid-flight attempt — no manual recalculation, no dependency on a future `visitor.identified` event. Deterministic and rules-based only; see `08-Security.md` §2.1 for why no LLM/agent involvement exists anywhere in this path.

**Milestone 4.1 Phase 2 implementation note (delivered) — CRM domain events now drive real Brain projections**: nine new event types (`contact.created`/`.updated`/`.deleted`, `company.created`/`.updated`/`.deleted`, `deal.created`/`.updated`/`.deleted`) are emitted transactionally by `createX`/`updateX`/`softDeleteX` in `packages/crm`, each via a dedicated `SECURITY DEFINER` function (`emit_contact_event`/`emit_company_event`/`emit_deal_event`, `03-Database-Architecture.md` §2.9) rather than an application-layer outbox write — the same "domain-state write and outbox row in one transaction" guarantee this section already requires, enforced at the SQL layer with a fixed event-type allowlist and a `current_org()` re-check that fails closed on any cross-tenant mismatch. Three new dispatcher `EventConsumer`s (`brain_projection_contact`/`_company`/`_deal`, `packages/brain`) drain these events using the same `workflow_runs` claim-lease idempotency pattern Milestone 3.4 established above — but the projection they perform is **state reconciliation, not payload replay**: on delivery, the consumer re-reads the entity's *current* row from `packages/crm` (including a soft-deleted row, via `getXByIdIncludingDeleted`) and deterministically recomputes its canonical profile, using the source row's own `updated_at` for freshness rather than event-arrival order — so a burst of redundant or out-of-order events for the same entity converges to the same correct result, never a stale overwrite. `docs/13-Technical-Design-Review.md` "Milestone 4.1 Phase 2" documents the full delivery history, including a first-insert concurrency defect found and fixed before acceptance.

**Milestone 4.1 Phase 3 implementation note (implementation accepted) — embedding write-back, still no provider call anywhere in this repository**: `brain_embeddings` gained a real write path — `POST /api/v1/brain/embeddings` (API-key-authenticated, `brain:embeddings:write` scope) accepts an already-computed 1536-dimension vector from an external caller (a future n8n workflow, not yet built) and persists it via `upsertEntityEmbedding` (`packages/brain`), reusing `upsertEntityProfile`'s exact `SELECT ... FOR UPDATE` + bounded-retry reconciliation design. Identity is deliberately narrow: **exactly one current embedding row per `brain_entity_profiles` row** (a canonical single-vector representation of that profile, not a multi-chunk document model), enforced by a partial unique index additive to Phase 1's own general, entity-profile-id-less chunk shape, which remains fully valid and untouched. Three new dispatcher consumers (`brain_embedding_trigger_contact`/`_company`/`_deal`) notify the future n8n workflow via the same nine events the Phase 2 projection consumers already drain — deliberately *not* mirroring `leadEnrichmentConsumer`'s "throw when unconfigured" pattern, since these consumers share Phase 2's own high-frequency events and an indefinite retry would head-of-line-block Phase 2's already-accepted delivery; an unconfigured webhook instead completes the delivery as a terminal, `workflow_runs`-observable failure, recoverable via `findProfilesNeedingEmbedding`'s backfill discovery pass (UUID-keyset, immune to the `timestamptz`-cursor precision class of bug by construction). `docs/13-Technical-Design-Review.md` "Milestone 4.1 Phase 3" documents the full delivery history, including two defects found and fixed during implementation itself.
...
- **Interpreter pattern** in the lead scoring engine (`packages/intelligence`, **implemented as of Milestone 3.4**) — `computeScore` evaluates each organization's own `scoring_rules` rows (a strict, allowlisted `{field, operator, value}` triple) against a fixed, hand-written evaluator; there is exactly one evaluation strategy, deterministic and rules-based only, no `eval`/`new Function`/dynamic SQL, and no AI/agent-assisted scoring exists or is planned as an alternate strategy (`08-Security.md` §2.1). An earlier draft of this document described a pluggable `RuleBasedScorer`/`AgentAssistedScorer` strategy split — superseded by the accepted Milestone 3.4 design, which deliberately rejected agent involvement in scoring as an anti-pattern, not merely deferred it.
