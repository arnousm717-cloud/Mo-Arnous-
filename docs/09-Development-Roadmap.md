...
**Milestone 4.1 Phase 1 status — Database + GDPR Foundation: ACCEPTED** (`docs/13-Technical-Design-Review.md` "Milestone 4.1 Phase 1"). `pgvector` enabled; the six `brain_*` storage tables shipped with tenant-safe RLS/grants/retention (`03-Database-Architecture.md` §2.9); `execute_contact_erasure()` extended for Brain data (`08-Security.md` §5.2). **Milestone 4.1 as a whole, and Phase 4 as a whole, remain in progress, not complete** — `packages/brain`, ingestion (event-driven or otherwise), the backfill/bootstrap step, sync-state integration, and every other feature below are not yet started.

**Features**
- **AI Revenue Brain foundation** (`11-AI-Revenue-Brain.md`): `packages/brain`, `pgvector` enabled (**done, Phase 1 above**), `brain_entity_profiles`/`brain_embeddings`/`brain_knowledge_documents`/`brain_sync_state` schema (**done, Phase 1 above** — the real shipped shape differs from this document's own original design, see `03-Database-Architecture.md` §2.9), ingestion wired to sources that already exist at this point (CRM, website visitors, tasks, manually-uploaded knowledge documents — **not** conversations: no `conversation_threads`/`chat_messages` table exists in this repository, `03-Database-Architecture.md` §2.4, correcting this document's own earlier assumption) — **not started** — built *before* the personas, since every persona now depends on it for shared context
