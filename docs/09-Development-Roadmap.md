...
**Milestone 4.1 Phase 1 status — Database + GDPR Foundation: ACCEPTED** (`docs/13-Technical-Design-Review.md` "Milestone 4.1 Phase 1"). `pgvector` enabled; the six `brain_*` storage tables shipped with tenant-safe RLS/grants/retention (`03-Database-Architecture.md` §2.9); `execute_contact_erasure()` extended for Brain data (`08-Security.md` §5.2). **Milestone 4.1 Phase 2 status — CRM Domain Events + Deterministic Projections: IMPLEMENTATION ACCEPTED** (`docs/13-Technical-Design-Review.md` "Milestone 4.1 Phase 2"). `packages/brain` now exists: nine CRM event types (`contact`/`company`/`deal` × `created`/`updated`/`deleted`), three `SECURITY DEFINER` emit functions, three dispatcher-driven projection consumers, deterministic canonical-profile projection (state re-read, not payload replay), soft-delete tombstones, GDPR hard-erasure cascade integration, and a resumable backfill/bootstrap CLI for pre-existing CRM data. **Milestone 4.1 as a whole, and Phase 4 as a whole, remain in progress, not complete** — no AI/model-provider dependency, embedding generation, semantic search, RAG, Brain API/UI/RBAC, or agent functionality of any kind exists yet; none of that is started.

**Features**
- **AI Revenue Brain foundation** (`11-AI-Revenue-Brain.md`): `packages/brain`, `pgvector` enabled (**done, Phase 1 above**), `brain_entity_profiles`/`brain_embeddings`/`brain_knowledge_documents`/`brain_sync_state` schema (**done, Phase 1 above** — the real shipped shape differs from this document's own original design, see `03-Database-Architecture.md` §2.9), ingestion wired to sources that already exist at this point — **CRM (contacts/companies/deals) done, Phase 2 above**; website visitors, tasks, and manually-uploaded knowledge documents remain **not started** (**not** conversations: no `conversation_threads`/`chat_messages` table exists in this repository, `03-Database-Architecture.md` §2.4, correcting this document's own earlier assumption). Embedding generation and semantic search remain **not started** — Phase 2 delivered structured canonical profiles only, no vector representation of them — built *before* the personas, since every persona now depends on it for shared context
