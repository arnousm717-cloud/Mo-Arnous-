...
**Milestone 4.1 Phase 1 status — Database + GDPR Foundation: ACCEPTED** (`docs/13-Technical-Design-Review.md` "Milestone 4.1 Phase 1"). `pgvector` enabled; the six `brain_*` storage tables shipped with tenant-safe RLS/grants/retention (`03-Database-Architecture.md` §2.9); `execute_contact_erasure()` extended for Brain data (`08-Security.md` §5.2). **Milestone 4.1 Phase 2 status — CRM Domain Events + Deterministic Projections: ACCEPTED** (`docs/13-Technical-Design-Review.md` "Milestone 4.1 Phase 2"). `packages/brain` now exists: nine CRM event types (`contact`/`company`/`deal` × `created`/`updated`/`deleted`), three `SECURITY DEFINER` emit functions, three dispatcher-driven projection consumers, deterministic canonical-profile projection (state re-read, not payload replay), soft-delete tombstones, GDPR hard-erasure cascade integration, and a resumable backfill/bootstrap CLI for pre-existing CRM data. **Milestone 4.1 Phase 3 status — Embedding Write-Back Foundation: IMPLEMENTATION ACCEPTED** (`docs/13-Technical-Design-Review.md` "Milestone 4.1 Phase 3"). `brain_embeddings` gained an additive, opt-in single-row-per-profile identity (`entity_profile_id`/`content_hash`/`source_version_at`), a concurrency-safe reconciliation write-back (`upsertEntityEmbedding`), a scoped service endpoint (`POST /api/v1/brain/embeddings`, `brain:embeddings:write`), dispatcher trigger consumers, and a profiles-needing-embedding backfill discovery pass — **this repository still does not call an embedding provider and does not generate any embedding itself**; Phase 3 only accepts and persists an already-computed vector. **Milestone 4.1 Phase 4 status — Tenant-Safe Semantic Retrieval Foundation: IMPLEMENTATION ACCEPTED** (`docs/13-Technical-Design-Review.md` "Milestone 4.1 Phase 4"). `searchEntityProfilesByEmbedding` and the scoped `POST /api/v1/brain/search` service endpoint (`brain:embeddings:read`) added exact (non-ANN) pgvector cosine-similarity retrieval over a caller-supplied, already-computed query vector — dual-layer tenant isolation, SQL-side freshness exclusion, deterministic ordering, and a minimized result shape; a zero-norm-vector defect (undefined cosine similarity, `minSimilarity`-threshold bypass) was found and classified HIGH during acceptance audit and corrected on both the query and stored-vector sides before final re-acceptance. **This repository still calls no embedding provider, accepts no query text, and performs no natural-language semantic search.** **Milestone 4.1 Phase 5 status — Automated Embedding-Trigger Recovery: IMPLEMENTATION ACCEPTED** (`docs/13-Technical-Design-Review.md` "Milestone 4.1 Phase 5"). This is an internal Milestone-4.1 implementation increment — distinct from, and not to be confused with, this roadmap's own separate "Phase 5 — Automations" below, which has not started. It gives Phase 3's own discovery pass (`findProfilesNeedingEmbedding`) a real, scheduled runtime caller (`GET /api/internal/brain-embedding-recovery`, `*/15 * * * *`), closing the gap Phase 3 itself disclosed — a missed live embedding-trigger notification previously had no automatic recovery path. A HIGH defect (a successful webhook 2xx response initially, incorrectly treated as permanent proof the embedding had materialized) was found during acceptance audit and corrected — a dedicated, cooldown-based claim function, no migration required — before final re-acceptance. **This repository still calls no embedding provider and holds no provider credential; Phase 5 only re-notifies the same not-yet-built n8n Brain Indexing workflow Phase 3 already targets.** **Milestone 4.1 as a whole, and this roadmap's own Phase 4 as a whole, remain in progress, not complete** — no AI/model-provider dependency, embedding *generation*, query-text-to-vector, RAG, Brain API/UI/RBAC, or agent functionality of any kind exists yet; none of that is started.

**Features**
- **AI Revenue Brain foundation** (`11-AI-Revenue-Brain.md`): `packages/brain`, `pgvector` enabled (**done, Phase 1 above**), `brain_entity_profiles`/`brain_embeddings`/`brain_knowledge_documents`/`brain_sync_state` schema (**done, Phase 1 above** — the real shipped shape differs from this document's own original design, see `03-Database-Architecture.md` §2.9), ingestion wired to sources that already exist at this point — **CRM (contacts/companies/deals) done, Phase 2 above**; website visitors, tasks, and manually-uploaded knowledge documents remain **not started** (**not** conversations: no `conversation_threads`/`chat_messages` table exists in this repository, `03-Database-Architecture.md` §2.4, correcting this document's own earlier assumption). **Embedding write-back (storage of an externally-computed vector) done, Phase 3 above; tenant-safe vector retrieval (exact cosine-similarity ranking over an already-computed query vector) done, Phase 4 above** — embedding *generation* and natural-language semantic search remain **not started**: no provider is called anywhere in this repository, `brain_embeddings.embedding` is populated only when an external caller supplies one, and `POST /api/v1/brain/search` accepts only a pre-computed vector, never query text — built *before* the personas, since every persona now depends on it for shared context
