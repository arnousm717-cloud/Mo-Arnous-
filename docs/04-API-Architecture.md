...
| Data Subject Requests | `/api/v1/data-subject-requests` | Staff | GDPR access/export/delete requests. **A `DELETE` on this resource is the only path that triggers the hard-delete/anonymization cascade in `03-Database-Architecture.md`'s deletion model** — see the note under §2's verb discipline below |
