...
- **Error responses use one consistent envelope** across every endpoint: `{ "error": { "code": "resource_not_found", "message": "human-readable, safe to display", "request_id": "..." } }`. `code` is a stable, documented machine-readable string (never a raw exception message); `request_id` correlates a client-reported issue to server-side logs/traces. **Concrete contract, as actually implemented (every shipped route as of Milestone 2.1: `organizations`, `consent`, `data-subject-requests`, `companies`, `contacts`)**: the real envelope is the flat `{ "error": "<string>" }` shown in each resource's own contract below (§2.1/§2.2/§2.4) — never the nested `{code, message, request_id}` structure above. This principle statement records the original design intent; it was never revisited to match what actually shipped, and no route in this codebase has ever emitted the structured form.
- **Idempotency keys are supported on every mutating request** (`POST`/`PATCH`), via an `Idempotency-Key` header. A retried request with the same key against the same endpoint returns the original result rather than creating a duplicate — this is what makes a client's network retry safe, and its absence would otherwise mean a flaky connection can silently duplicate a deal or contact. **Concrete contract, as actually implemented (Milestone 2.1F-A, `public.idempotency_keys`)**: the header is **optional**, not required — a request without one is simply not deduplicated, never rejected. Same key + identical request body → the original response is replayed byte-for-byte, the mutation is never re-executed. Same key + a *different* request body → `409 Conflict` (`{ "error": "Idempotency-Key already used with a different request" }`), never silently applied as a second write. Keys are scoped per organization (never collide across tenants) and expire after **24 hours**, after which the same key becomes reusable. Neither the raw header value nor the raw request body is ever persisted — only SHA-256 hashes/fingerprints. **Since wired into live routes (Milestone 2.1F-B)**: `POST`/`PATCH /api/v1/companies` and `/api/v1/contacts` are the actual, shipped consumers of this mechanism — see §2.4.
...
Session auth only (no API-key path yet, same as every other route above). Error envelope is always flat — `{ "error": "<string>" }` — never a second, more structured shape; this correction to the general resource-map description in §2 was found necessary while auditing the actual code (2.1F audit), not designed up front.
...
- **`GET /api/v1/companies/{id}`** / **`GET /api/v1/contacts/{id}`** — `{ company }` / `{ contact }`, `200`. A cross-org id and a genuinely nonexistent id are **indistinguishable** — both `404`, same body — same "don't confirm a foreign tenant's row exists" rule as `data-subject-requests/{id}` (§2.2).
