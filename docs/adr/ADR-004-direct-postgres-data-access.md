...
- This decision is scoped to the app's own internal data access, not the public API surface: an *external* integrator or n8n workflow never uses PostgREST or a direct Postgres connection either — both go through the app's own authenticated `/api/v1/*` API, using a `service`-scoped `api_keys` credential (ADR-002, `04-API-Architecture.md` §8).
