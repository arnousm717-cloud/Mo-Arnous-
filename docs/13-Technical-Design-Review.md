...
of the `visitor.identified` outbox event exists yet, as of this
milestone's own close. **Milestone 3.3 — Lead Enrichment (AI Revenue OS
side) has since shipped** — see "Milestone 3.3 — Overall Closeout"
below; the `visitor.identified` outbox event now has its first real
consumer.

---

## Milestone 3.3 — Overall Closeout

**Milestone 3.3 — Lead Enrichment, AI Revenue OS side.** **Status:
COMPLETE — pending commit.** Delivered as sub-phases 3.3A through 3.3F
(enrichment/`workflow_runs` schema + RLS, service-authentication
function, the `packages/auth` service-actor/scope layer, the
provider-agnostic enrichment domain layer, the two API-key-authenticated
write-back endpoints, the cron-driven dispatch route), followed by
**four** sequential, independent, read-only Final Implementation
Acceptance Audits — the first three each found exactly one new finding
(two HIGH, one MEDIUM) and each was remediated in a dedicated,
narrowly-scoped pass before the next audit; the fourth found nothing
further and returned **GO**. As of this closeout the working tree is
clean against `origin/main` (`f43ac14d6bf2408bd1ae495edd664e1e77da474d`)
with the milestone's files staged as untracked/modified — **not yet
committed or pushed**.

### Architecture

Provider-agnostic by construction: this platform never holds a provider
credential and never calls a provider directly. n8n calls the enrichment
provider on its own side and pushes an already-normalized result back via
the write-back endpoints below. Provider selection was explicitly
**deferred** at the accepted architecture-resolution stage — building the
AI Revenue OS side provider-agnostically first, with provider-specific
integration left to a later n8n workflow-authoring sub-phase — a
deliberate scope boundary, not an oversight. `contact.created`/
`company.created` are documented (`docs/06` §2) as alternate future
triggers but are not wired; the one real, firing trigger today is
`visitor.identified` (Milestone 3.2), the only event source that
concretely exists.

Service-to-service authentication is a genuinely separate mechanism from
staff session auth — deliberately not `resolveRequestContext()`/`can()`,
since a machine credential is not a human staff role. `resolve_api_key()`
(`SECURITY DEFINER`, no `auth.uid()` guard, mirroring
`resolve_tracking_site()`'s own precedent exactly) resolves a pre-hashed
Bearer token to `(api_key_id, organization_id, scopes)`; `organization_id`
is taken exclusively from the matched row, never a caller-supplied value.
Authorization is scope-based (`hasScope`), reading `api_keys.scopes` — a
jsonb column that has existed, unused, since M1.7.

### Database (`docs/03-Database-Architecture.md` §2.3/§2.5/§2.10)

- `contact_enrichment`/`company_enrichment` (new) — one row per
  `(organization_id, entity, provider)`, upserted in place, never
  accumulated as history (unlike this codebase's own append-only audit
  tables) — a stale row has no evidentiary value once superseded.
  Composite tenant-safe FK to `contacts`/`companies`, `ON DELETE CASCADE`
  — an enrichment row cannot outlive its subject's hard erasure, verified
  end-to-end with a real GDPR data-subject-request erasure.
- `workflow_runs` (new) — narrower than this document's own
  originally-documented shape (§2.5): no `workflow_ref_id` FK, since
  `workflow_refs`/`integration_connections` do not exist yet. `workflow_key`
  is a static, code-defined identifier instead, the same discipline
  `event_deliveries.consumer` already established. `UNIQUE (organization_id,
  workflow_key, source_event_id)` is the real per-logical-operation
  idempotency guard — a retried delivery attempting to re-record "this
  same trigger succeeded" is structurally a no-op, proven under genuine
  concurrent writes (two real simultaneous connections racing the
  identical upsert), never a duplicate cost/observability entry.
- `event_deliveries` (M1.7, additively extended) — gained `status`
  (`'leased'`/`'delivered'`) and `lease_expires_at` (Milestone 3.3
  Claim-Lease Remediation, `20260902090000`, additive, does not edit the
  original M1.7 migration). Existing rows backfilled to `'delivered'`
  (the old semantics' own meaning) via the column default; every fresh
  acquisition always specifies `status='leased'` explicitly, never
  relying on that default.
- `resolve_api_key(p_key_hash text) returns table(api_key_id, organization_id,
  scopes)` (`SECURITY DEFINER`, `20260901090400`) — single atomic
  `UPDATE ... RETURNING`, bumps `last_used_at`, returns zero rows
  (never a distinguishable error) for a nonexistent, revoked, or
  hash-mismatched key.

### Runtime — the dispatcher, after two remediation rounds

`packages/database/src/events.ts`'s `dispatchPendingEvents()` reads at
most `DISPATCH_BATCH_SIZE` (10, fixed, code-defined) pending events per
call, scoped to the calling consumers' own registered event types. Per
`(event, consumer)` pair, a single atomic statement both acquires a fresh
lease and reclaims an expired one:

```sql
insert into event_deliveries (event_id, consumer, status, lease_expires_at)
values ($1, $2, 'leased', now() + (120 seconds))
...
No advisory or session lock of any kind protects this — the unique
constraint plus this one statement is the entire concurrency-safety *and*
crash-recovery mechanism, proven under genuine two-connection concurrency
(both a fresh-acquire race and a stale-lease-reclaim race — exactly one
connection ever wins either). No transaction spans any external HTTP
call. On success, a separate, independently persisted statement
transitions the row to `'delivered'` — terminal, never reclaimable again
by construction. On a caught failure, the row is deleted immediately
(retryable on the very next tick, no lease wait); a process crash instead
leaves the lease in place, self-healing once it expires via the same
statement — no separate sweeper process exists or is needed.

`GET /api/internal/dispatch-events` (Vercel Cron, every minute,
`CRON_SECRET`-protected) is the sole caller in production, registering
one consumer — `lead_enrichment` (`visitor.identified` only) — which
posts `{eventId, organizationId, entityType, entityId}` to an
operator-configured n8n webhook URL. `eventId` is `event.id` verbatim,
stable across every retry of the same pair, the mechanism downstream
processing can use to deduplicate a redelivered trigger — this
architecture is **at-least-once delivery, not exactly-once external side
effects**, an unavoidable distributed-systems boundary it does not claim
to close.

`events.processed_at` (an observability convenience only, never the real
idempotency mechanism — that is `event_deliveries`' own unique
constraint) is set only once every consumer applicable **to the current
dispatcher invocation** has its own terminal `status='delivered'` row for
that exact `(event_id, consumer)` pair — a completion count explicitly
scoped to `consumer = ANY(currentApplicableConsumerNames)`, never
influenced by a historical, renamed, removed, or otherwise non-applicable
consumer's own unrelated delivered row (the Third Final Implementation
Acceptance Audit's own finding and fix).

### API surface (`docs/04-API-Architecture.md` §2.3, new §2.8)

`POST /api/v1/contacts/{id}/enrichment`, `POST /api/v1/companies/{id}/
enrichment` — API-key auth (`enrichment:write` scope, `Authorization:
Bearer arev_...`), 64KB genuinely byte-counted bounded body (not merely
trusting a declared `Content-Length`), strict field allowlist
(`workflowKey` has no extraction path at all — always the fixed
server-side `'lead_enrichment'` constant), 30/minute-per-organization
rate limit (reuses `check_tracking_rate_limit()` under a verified-collision-
free namespaced bucket). A live re-check of the target contact/company
(existence and `deleted_at IS NULL`) runs inside the same transaction as
the write — a delayed provider response arriving after erasure, or after
the subject was recreated under a new id, is rejected identically to any
other reason, never silently reattached to the wrong row. The monotonic
upsert (`WHERE excluded.fetched_at > table.fetched_at`, strict, not `>=`)
makes equal-timestamp replacement deterministic — the first-stored row
always wins, never arrival-order-dependent.

### Verification

**Four** independent, read-only, adversarial Final Implementation
Acceptance Audits were performed in sequence:

1. Found **HIGH** — the original dispatcher held a batch-wide transaction
   and an advisory lock open across every consumer's external HTTP call,
   unbounded by any `LIMIT`. Remediated: bounded batch, no lock, no
   transaction spanning external I/O, event-type-scoped selection.
2. Found **HIGH** — the remediation's own "claim-first" design left a
   real crash window causing silent, permanent, unretried delivery loss.
   Reproduced directly against real Postgres. Remediated: the
   claim/lease/terminal-delivery model described above (additive
   migration).
3. Found **MEDIUM** — the `processed_at` completion check could be
   satisfied by an unrelated historical/renamed/removed consumer's own
   delivered row. Reproduced directly. Remediated: completion scoped to
   the current invocation's own deduplicated applicable consumer names.
4. Found **nothing further**. Independently reproduced all twelve
   hostile scenarios named in its own brief against real PostgreSQL —
   not trusting any prior report — and returned **GO**.

- `pnpm turbo run test --force` (normal parallelism, no
  `--no-file-parallelism`/`--concurrency` workaround): **2788/2788**,
  reproduced fresh and again against a real accumulated backlog, both
  fully green.
- `pnpm lint`/`typecheck`/`build`: clean across all 9 packages.
- Fresh `supabase db reset`: all six Milestone 3.3 migrations (five
  original plus the additive lease-state migration) apply cleanly, in
  order, from a fully torn-down local database — performed repeatedly
  across all four audits and both remediation passes.
- Dedicated regression suites at final sign-off: dispatcher **21/21**,
  ...
  enrichment **21/21**, contact erasure **24/24**.

### Remaining Findings (preserved, not fixed this milestone)

- **Informational** — `contact.created`/`company.created` triggers remain
  unwired; only `visitor.identified` fires today. Correctly scoped as
  complete for this milestone's own authorized initial slice, not a
  partial implementation of a broader one (confirmed by direct search: no
  domain code emits either event).
- **Informational** — `raw_payload`'s `expires_at` is computed and
  stored but nothing yet reads or enforces it as an active purge.
- **Informational** — the dispatcher's inner `if (applicable.length === 0)
  continue` branch is structurally unreachable dead code, a consequence
  of the first remediation round's own event-type-scoped `SELECT` —
  confirmed by direct proof (not merely assumed), left in place since it
  documents intent cheaply and touching it was out of scope for the fix
  that made it unreachable.
- **Informational** — a small number of pre-existing, unrelated
  tracking-rate-limit tests are intermittently flaky under heavy
  concurrent full-suite load, independently reproduced across multiple
  audit sessions; none were touched by this milestone, and none
  manifested in final sign-off verification.

**Milestone 3.3: PASS.** Lead enrichment write-back, service
authentication, and the reliability-hardened dispatcher are live; the
real n8n workflow JSON, provider selection, and any provider credential
remain out of this milestone's own scope, deferred to a later n8n
workflow-authoring sub-phase.
...
## Milestone 4.1 Phase 1 — Database + GDPR Foundation

**Milestone 4.1 — Brain Foundation.** **Status: Phase 1 (Database + GDPR
Foundation) ACCEPTED — pending commit. Milestone 4.1 as a whole, and
Phase 4 as a whole, remain IN PROGRESS, not complete.** Delivered as a
single implementation turn (pgvector, six-table Brain schema, RLS/grants,
retention registration, GDPR erasure integration, database/adversarial
tests), followed by a Final Implementation Acceptance Audit that
returned **NO-GO** on two independently-reproduced defects, a targeted
fix round addressing both, and a Final Re-Acceptance Audit that
independently re-verified the fixes and returned **GO**. As of this
entry the working tree is clean against `origin/main`
(`cbafb5e4283978dc4439e6d5e3674aa42c35dff7`) with the phase's files
staged as untracked/modified — **not yet committed or pushed**.

### Scope actually authorized and delivered

Database schema, tenant isolation, grants, retention registration, and
GDPR/DSR erasure integration only. Explicitly **not** in scope this
phase and **not built**: `packages/brain`, CRM domain-event emission
for Brain ingestion, the backfill/bootstrap script, `brain_sync_state`
integration, embedding generation, semantic search, any AI/model-
provider dependency, Brain API routes/Server Actions/UI/RBAC
permissions, and agent functionality of any kind.

### Schema

Six new tables (`docs/03-Database-Architecture.md` §2.9):
`brain_knowledge_documents`, `brain_entity_profiles`,
`brain_entity_profile_history`, `brain_embeddings`,
`brain_embedding_entity_refs`, `brain_sync_state`. The entity-reference
design deliberately deviates from this document's own original
`docs/11-AI-Revenue-Brain.md` §4 proposal (an unstructured
`brain_embeddings.entity_refs` jsonb array) in favor of a **relational,
three-nullable-composite-FK** design — `contact_id`/`company_id`/
`deal_id` (exactly one set, enforced by a `CHECK` matching
`entity_type`) plus real composite tenant-safe FKs, `ON DELETE CASCADE`,
per entity kind, and a new `brain_embedding_entity_refs` junction table
for the many-to-many case — a stronger, DB-enforceable invariant over an
application-trusted jsonb convention, at the accepted cost of not
scaling cleanly past ~4-5 entity types. A previously-unverified schema
gap was discovered and fixed as a prerequisite: `public.deals` had no
`unique(organization_id, id)` constraint (unlike `companies`/`contacts`/
`pipelines`/`pipeline_stages`), so no composite FK could target it —
closed with the exact precedent already established for `contacts`
(`20260814100000`), verified to fail against zero existing data.
`brain_embeddings.embedding` is `vector(1536)`, nullable, schema-only —
no provider selected, no row ever populated, no similarity index
created.

### Tenant isolation and grants

Every Brain FK carrying a tenant-owned parent is a real composite
`(organization_id, id)` FK, adversarially verified against real
Postgres (not merely designed): cross-tenant parent attacks against
`brain_entity_profiles`, `brain_embedding_entity_refs`, and
`brain_embeddings` (via `brain_entity_profile_history`'s and
`brain_embedding_entity_refs`'s own parent FKs) were all attempted live
and rejected by the FK layer, independent of RLS. RLS is enabled on all
six tables; `authenticated` holds ordinary `SELECT`/`INSERT`(/`UPDATE`
on the three mutable tables) — no `DELETE` grant anywhere — with one
deliberate exception: `brain_knowledge_documents` is **SELECT-only**
for `authenticated` (see Knowledge documents, below). `anon` holds zero
grants on any Brain table.

### GDPR/DSR erasure — targeted-capture design (post-fix)

The first implementation extended `execute_contact_erasure()` with a
purge step that deleted a `brain_embeddings` row only if it currently
had zero `brain_embedding_entity_refs` rows. The Final Implementation
Acceptance Audit reproduced two live defects in this design against
real Postgres: (1) **BLOCKER** — a chunk shared between the erased
contact and a surviving company/deal ref was never deleted at all, so
the erased contact's own personal data (name/email/phone, in the
reproduction) remained fully readable in `chunk_text` after erasure;
(2) **HIGH** — the "delete if currently orphaned" predicate was scoped
to the whole organization, not to this specific erasure, so it could
delete an unrelated, already-orphaned embedding that had nothing to do
with the erased contact.

The fix round replaced the design: `execute_contact_erasure()` now
captures the exact set of `brain_embeddings` ids linked to the target
contact (via `brain_embedding_entity_refs`) **before** the contact
delete fires — while the linking evidence still exists — then, after
the delete (which cascades away the contact's own FK-linked Brain rows
structurally), deletes every captured embedding **in full**, even one a
company/deal ref also points at. Deleting the embedding row cascades
away any remaining refs on it. Because the delete is keyed by the
captured id set rather than by "currently has zero refs," it can never
reach an unrelated orphan or a knowledge-document-sourced chunk (which
never receives an entity ref at all, by construction).

Both the original defects and the fix were independently reproduced
against live Postgres during the Final Re-Acceptance Audit: a hostile
fixture with synthetic PII (name/email/phone) in a chunk shared between
the erased contact and a company confirmed the chunk is now deleted in
full and the PII unrecoverable anywhere in the organization; a same-
organization unrelated zero-ref embedding, and one with its own valid
company ref, both confirmed to survive byte-for-byte; a cross-
organization embedding confirmed untouched; an empty-capture-set
erasure (a contact with no Brain data at all) confirmed to succeed with
no SQL error. The new regression tests were independently proven to
fail against the original defective implementation (reinstalled live,
never written to a file) before being confirmed to pass against the
fix. `preview_contact_erasure()` was deliberately not extended, matching
the established precedent that neither prior real cascade extension
(2.3A, 3.2F) touched it either.

### Knowledge documents — schema-only, no write path

`brain_knowledge_documents.content_text` is free text with no
deterministic entity linkage, so it cannot participate in the
FK-driven/targeted-capture erasure design above. The Final Implementation
Acceptance Audit flagged the original design (ordinary `authenticated`
INSERT/UPDATE grants, no retention row, no erasure coverage) as a real,
if not yet exploitable, compliance-scope gap — no application/API/UI
write path exists, but the Postgres-level grant did. The fix round
removed the INSERT/UPDATE grant and policy entirely, leaving this table
**SELECT-only** for `authenticated` until a later milestone ships a
compliant ingestion/DSR design (most likely a narrow `SECURITY DEFINER`
write path with real content validation). Live-tested: `authenticated`
SELECT succeeds; INSERT/UPDATE/DELETE/TRUNCATE all fail with a genuine
grant-level `permission denied`, not an RLS/`current_org()`-unset
artifact. No seed script or service-role path writes to this table
today, independently verified by source search.

### Retention

`brain_entity_profiles`/`brain_entity_profile_history`/
`brain_embeddings`/`brain_embedding_entity_refs` registered in
`data_retention_policies` (2555-day platform default, verified live
after a fresh reset). `brain_knowledge_documents` (no write path) and
`brain_sync_state` (a cursor/watermark, no personal data) deliberately
excluded, mirroring the `tags`/`taggings` precedent.

### Future-ingestion invariant (documented, not enforced)

`brain_entity_profiles.profile` (jsonb) carries no schema-level
constraint against a company/deal profile embedding a specific
contact's personal data — a comment directly on the column documents
the invariant a future ingestion design must honor (deterministic
contact provenance, or no cross-entity PII at all) before this column
is ever populated. Comment-only, verified not to introduce any
constraint, trigger, or behavior change — confirmed by full-file
byte-level comparison against the pre-comment version.

### Audit trail

Three independent read-only audits ran against this implementation, in
order: a Final Implementation Acceptance Audit (**NO-GO** — one
BLOCKER, one HIGH, both reproduced live against real Postgres, not
accepted on the strength of the implementation's own report), a
targeted fix round (both defects fixed, three mandatory hostile
regression tests added and proven falsifiable, knowledge-document
grants corrected), and a Final Re-Acceptance Audit (independently
re-derived every claim from source and fresh live reproduction rather
than trusting the fix round's own report — **GO**, zero BLOCKER, zero
HIGH remaining). A subsequent read-only comment-cleanup pass corrected
two LOW-severity stale SQL comments left behind by the fix round
(describing the replaced "purge orphans" design instead of the shipped
targeted-capture design) — proven comment-only via full-file
comparison, re-validated, **GO**.

### Final validation (Final Re-Acceptance Audit, fresh)

`packages/database` full suite **825/825** passed (35 files, up from
819 pre-fix — 71 new Brain tests plus regression coverage added during
the fix round). `packages/compliance` full suite **52/52** passed, no
regression. Full monorepo test suite (cache bypassed): **8/8 tasks
successful**, `apps/web` **1156/1156** passed. `pnpm lint`/`typecheck`
(cache bypassed): **8/8 packages clean**. `git diff --check` clean
throughout.

### Deviations from the M4.1 Detailed Design

1. `deals` gained `unique(organization_id, id)` — a schema gap
   discovered during implementation, not anticipated by the Detailed
   Design, fixed with an exactly-precedented additive prerequisite.
2. Entity-reference design: relational three-nullable-composite-FK plus
   a junction table, not the Detailed Design's/`docs/11`'s own original
   jsonb-array proposal — see Schema, above.
3. GDPR erasure design: targeted-capture, full-artifact deletion — not
   the Detailed Design's originally-proposed "purge if orphaned"
   approach, which the acceptance audit proved defective; see GDPR/DSR
   erasure, above.
4. `brain_knowledge_documents` shipped SELECT-only for `authenticated`,
   not the Detailed Design's assumed ordinary RLS-scoped write grant —
   a conservative correction made during the fix round, not part of the
   original implementation authorization.

**Milestone 4.1 Phase 1: PASS.** Database schema, tenant isolation,
grants, retention, and GDPR erasure integration for the AI Revenue
Brain are live, adversarially verified, and free of BLOCKER/HIGH
findings. `packages/brain`, ingestion, embedding generation, semantic
search, and every agent-facing capability remain unbuilt — Milestone
4.1 is not complete, and Phase 4 has not materially progressed beyond
this one foundation phase.

## Milestone 4.1 Phase 2 — CRM Domain Events + Deterministic Projections

**Milestone 4.1 — Brain Foundation.** **Status: Phase 2 (CRM Domain
Events + Deterministic Projections) IMPLEMENTATION ACCEPTED. Milestone
4.1 as a whole, and Phase 4 as a whole, remain IN PROGRESS, not
complete.** Delivered as a single implementation turn (one migration
adding three `SECURITY DEFINER` emit functions, CRM event emission
wired into 9 call sites across `contacts`/`companies`/`deals`, a new
`packages/brain` domain package, dispatcher registration, a backfill
CLI, and new tests), followed by two independent read-only audit
rounds, each of which returned **NO-GO** on a genuine, live-reproduced
defect, a targeted correction round for each, and a final, most
rigorous Final Implementation Re-Acceptance Audit that independently
re-derived every claim from source and fresh live reproduction rather
than trusting any prior round's own report — **GO**. As of this entry
the working tree remains clean against `origin/main`
(`78f6a8f0ce6c0945a52e7f62b760e1dfd216acf6`) with the phase's files
staged as untracked/modified — **not yet committed or pushed**.

### Scope actually authorized and delivered

CRM domain-event emission (transactional, `SECURITY DEFINER`-gated),
the `packages/brain` domain layer (canonical-profile types, a
deterministic projector, a repository with concurrency-safe upsert,
an event-driven ingestion/consumer layer, a resumable backfill CLI),
and dispatcher registration for three new consumers. Explicitly **not**
in scope this phase and **not built**: any AI-provider dependency,
embedding generation, semantic search, the retrieval client/tools
(`11-AI-Revenue-Brain.md` §5), the continuous-learning loop (§6),
ingestion for any non-CRM source (website visitors, tasks, knowledge
documents), and Brain API routes/Server Actions/UI/RBAC permissions.

### Event emission

Three new `SECURITY DEFINER` functions (migration `20260906090000`),
`emit_contact_event`/`emit_company_event`/`emit_deal_event`, are the
sole write path into `public.events` for nine new event types
(`contact.created`/`.updated`/`.deleted`, `company.*`, `deal.*`) —
`03-Database-Architecture.md` §2.9. Each validates its event type
against a fixed in-function allowlist, derives `organization_id` from
the target row, and independently re-checks
`v_<entity>.organization_id is distinct from current_org()` before
inserting, failing closed on any mismatch — a cross-tenant
event-forgery hardening discovered and fixed during the original
implementation turn, before any audit ran, not a later correction.
Payloads are ID-minimized (`organization_id` plus the entity id only),
by design, so a consumer must re-read current state rather than trust
a payload snapshot. Called transactionally from `packages/crm`'s
`createX`/`updateX`/`softDeleteX` across all three entities, guarded
so a no-op update never emits.

### `packages/brain` — canonical profiles via state reconciliation

New workspace package, dependent only on `@ai-revenue-os/crm` and
`@ai-revenue-os/database` (never `auth` — `02-Software-Architecture.md`
§4's package-boundary table corrected to match). `projectContactProfile`/
`projectCompanyProfile`/`projectDealProfile` deterministically build a
`CanonicalContactProfile`/`CanonicalCompanyProfile`/`CanonicalDealProfile`
(`profileVersion: 1`) from the entity's *current* row — never from a
historical event payload — with a fixed, reconstructed key order so
jsonb key-reordering can never produce a spurious diff. `upsertEntityProfile`
writes `brain_entity_profiles` (current row) and
`brain_entity_profile_history` (insert-only) together. The three
dispatcher `EventConsumer`s (`brain_projection_contact`/`_company`/
`_deal`) reuse `workflow_runs`' claim-lease idempotency pattern exactly
as Milestone 3.4's `lead_scoring_post_enrichment` recovery path
established — an at-least-once, idempotently-processed contract, never
exactly-once. A soft-deleted entity projects with `isDeleted: true` and
content preserved (via `getXByIdIncludingDeleted`, added for `deals` to
match the pre-existing `contacts`/`companies` variants); a subsequent
GDPR hard erasure removes the projected rows structurally via the same
`ON DELETE CASCADE` FKs Phase 1 established, proven that a stale
`.deleted` event replayed after hard erasure cannot resurrect anything.
`bootstrapBrainForOrganization` (backfill, `PAGE_SIZE=50`) pages through
pre-existing CRM data via `packages/crm`'s cursor pagination, resumable
from `brain_sync_state.cursor`.

### Defect 1 (BLOCKER) — first-insert projection concurrency race

The Final Implementation Acceptance Audit found and live-reproduced a
race in `upsertEntityProfile`'s no-existing-row branch: a lost
`INSERT ... ON CONFLICT DO NOTHING` returned a bare stale result,
permanently letting stale data win if it physically lost the race for
first-insert. **NO-GO.** The correction round replaced this with a
bounded 2-attempt reconciliation loop, mathematically sufficient for
any N-way race because Postgres blocks a conflicting `INSERT` until the
winning transaction resolves. Proven via differential testing: the new
concurrency test was confirmed to fail against the reinstalled original
defective code, then confirmed to pass against the fix — the same
"revert, confirm failure, restore, confirm pass" standard Phase 1's own
GDPR-erasure regression tests used. An initial test-timing-stagger
technique was itself proven unreliable (any offset large enough to bias
the race outcome was also large enough for the head-started call to
fully commit first, silently missing the buggy path) and replaced with
genuinely-simultaneous `Promise.all` concurrency, verified reliable
(29/30 trials) via a raw two-connection probe.

### Defect 2 (BLOCKER) — pagination cursor microsecond-precision loss

Discovered as a side effect of writing a bulk-insert backfill test, then
independently reproduced and escalated to BLOCKER by the Final
Re-Acceptance Audit: node-postgres's default `timestamptz` type parser
returns a JS `Date` (millisecond precision only), while Postgres itself
stores and compares `created_at` at microsecond precision
(`packages/crm/src/pagination.ts`'s own header comment has the full
technical account). Building a keyset cursor from that truncated value
can silently skip a row that shares a millisecond with the page-boundary
row — reproduced directly against real Postgres. The audit explicitly
declined to treat this as "pre-existing, therefore lower severity,"
since Phase 2's own `backfill.ts` directly depends on the shared
`pagination.ts` helper this bug lives in — **NO-GO**, a second time, on
this single defect. The dedicated correction round fixed it at the root
across all 8 consumers of the shared helper (contacts, companies, deals,
activities, notes, pipelines, tags ×2): every paginated list query
additionally selects `created_at::text as created_at_cursor`, and
`buildPage` builds the cursor from that full-precision text value —
never a wider-page, offset-pagination, rounding, or test-only-stagger
workaround, all of which were explicitly considered and rejected.
Verified via differential testing per entity, and via a real,
unstaggered collision fixture exactly at the backfill's own
`PAGE_SIZE=50` boundary, replacing an earlier test-only stagger
workaround that the Final Re-Acceptance Audit had correctly flagged as
capable of masking the real bug. Cursor shape and `decodeCursor`
validation are unchanged, so an already-issued cursor still decodes and
works — assessed as INFORMATIONAL, not a blocker, since Phase 2 was
never deployed prior to this fix, so no Phase-2-issued cursor predating
it can exist.

### Other findings, both corrected

- **MEDIUM — dispatcher test-retry budget too tight under full-suite
  load**: `apps/web/tests/dispatch-events-api.test.ts`'s bounded drain
  budget (20 attempts) became insufficient once every CRM test fixture
  began emitting a real event, inflating ambient pending-event backlog.
  Reproduced twice under concurrency-1 full-monorepo runs. Fixed by
  measuring the actual ambient backlog and raising the budget to 150
  (10×+ headroom), with affected test timeouts raised to 90000ms — a
  test-reliability fix, not a production dispatch behavior change.
- **MEDIUM — missing multi-page/resume backfill coverage**: fixed with
  genuine >`PAGE_SIZE` fixtures and real resume-from-cursor tests using
  only already-exported functions, no mocks.
- **LOW — weak hard-erasure test**: originally used a fabricated
  nonexistent UUID rather than a genuinely-profiled-then-erased contact;
  fixed with real `execute_contact_erasure()` flows, including a
  soft-delete → hard-erasure → stale-`.deleted`-replay sequence.

### Audit trail

Four read-only audits/corrections ran against this implementation, in
order: a Final Implementation Acceptance Audit (**NO-GO** — one
BLOCKER, the first-insert concurrency race, plus 2 MEDIUM and 1 LOW,
all reproduced live), a correction round (all four fixed, differential
testing applied throughout), a Final Re-Acceptance Audit that
independently reproduced the pagination bug and escalated it to
BLOCKER on Phase-2-dependency grounds (**NO-GO**, a second time), a
dedicated pagination-only correction round (root-cause fixed across all
8 shared-helper consumers), and a final Final Implementation
Re-Acceptance Audit — explicitly instructed not to trust any prior
report — that independently re-verified every prior fix via fresh live
security probes, fresh multi-run test execution, and a new, harder
independent pagination reproduction (2 separate collision pairs)
(**GO**, zero BLOCKER/HIGH remaining). This history is recorded in
full, not condensed away: two of the four audit/correction rounds
returned NO-GO before final acceptance.

### Final validation (Final Implementation Re-Acceptance Audit, fresh)

Full monorepo test suite: **3153/3153** passed. `pnpm lint`/`typecheck`/
`build`: clean across the monorepo. `pnpm audit --audit-level=high`:
completed against the live registry endpoint with no known
vulnerabilities (registry timeouts had made this incomplete in earlier
turns; this final run genuinely completed). `pnpm-lock.yaml`'s only
diff is the new `packages/brain` workspace-importer block and
`apps/web`'s link to it — zero new external dependency versions.
`git diff --check` clean throughout.

### Deviations from the M4.1 Phase 2 authorization

1. The cross-tenant `current_org()` re-check in the three emit
   functions was added during the original implementation turn itself,
   before any audit ran — a hardening beyond the minimum, not a
   correction-round fix.
2. The pagination-precision fix's scope was widened from the 3
   Phase-2-relevant entities (contacts/companies/deals) to all 8
   consumers of the shared `pagination.ts` helper (also activities,
   notes, pipelines, tags ×2) — every consumer of a shared defective
   helper was fixed, not only the ones this phase directly touches.
3. `deals.ts` gained `getDealByIdIncludingDeleted`, mirroring the
   pre-existing `contacts`/`companies` variants — a small additive
   parity fix, not part of the original Detailed Design's explicit
   file list.

**Milestone 4.1 Phase 2: PASS.** CRM domain-event emission, the
`packages/brain` deterministic-projection domain layer, dispatcher
integration, soft-delete/GDPR-erasure interaction, and backfill are
live, adversarially verified across two full audit-correction cycles,
and free of BLOCKER/HIGH findings. Embedding generation, semantic
search, the retrieval client, and every agent-facing capability remain
unbuilt — Milestone 4.1 is not complete, and Phase 4 has not materially
progressed beyond these two foundation phases.

## Milestone 4.1 Phase 3 — Embedding Write-Back Foundation

**Milestone 4.1 — Brain Foundation.** **Status: Phase 3 (Embedding
Write-Back Foundation) IMPLEMENTATION ACCEPTED. Milestone 4.1 as a
whole, and Phase 4 as a whole, remain IN PROGRESS, not complete.**
Delivered as a single controlled implementation turn, followed by one
strict read-only Final Implementation Acceptance Audit that
independently re-derived every claim from source, live Postgres
probes, and fresh command runs rather than trusting the implementation
turn's own report — **GO**, zero BLOCKER, zero HIGH. As of this entry
the working tree remains clean against `origin/main`
(`f2340ad4c45ae2bc604a80661d17d7296622c903`) with the phase's files
staged as untracked/modified — **not yet committed or pushed**.

### Scope actually authorized and delivered

An additive schema extension to `brain_embeddings`, a concurrency-safe
domain write-back function, a scoped API-key-authenticated service
endpoint, dispatcher trigger consumers, and a profiles-needing-
embedding backfill discovery pass — the persistence/notification layer
around an embedding, not the embedding itself. Explicitly **not** in
scope this phase and **not built**: any embedding-provider SDK or
credential in this repository, the actual n8n Brain Indexing workflow,
automatic scheduling of the backfill, semantic/vector search, the
retrieval client/tools, knowledge-document/email/meeting multi-chunk
ingestion, the continuous-learning loop, Brain UI, a general end-user
Brain API, any new Brain-specific RBAC permission, and agent
functionality of any kind.

### Schema evolution — additive, opt-in, non-destructive

One migration (`20260907090000`) adds three nullable columns to
`brain_embeddings`: `entity_profile_id` (composite tenant-safe FK to
`brain_entity_profiles`, `ON DELETE CASCADE`), `content_hash`,
`source_version_at`; a conditional `CHECK`
(`entity_profile_id is null or (content_hash is not null and
source_version_at is not null)`); and a partial unique index on
`(organization_id, entity_profile_id) WHERE entity_profile_id IS NOT
NULL`. Every column is nullable specifically so Phase 1's own original,
more general design — an `entity_profile`-sourced chunk referencing
zero, one, or several entities purely via `brain_embedding_entity_refs`,
with no `entity_profile_id` at all — remains fully valid and untouched;
live-verified against the migration (a general-shape insert succeeds
with none of the three new columns set), and every pre-existing
Phase 1/2 test exercising that shape passes unmodified.

### Single-row-per-profile decision (the acceptance audit's MEDIUM finding, now resolved by documentation)

When `entity_profile_id` **is** set — the only path
`upsertEntityEmbedding` ever writes — the schema enforces **exactly
one current embedding row per entity profile**, live-proven via a
direct unique-violation reproduction. This is a deliberate, narrower
identity layered on top of Phase 1's general design: a single canonical
vector representation of one structured `brain_entity_profiles` row's
content, not a multi-chunk document-splitting model. The Final
Implementation Acceptance Audit found this constraint was internally
consistent across schema, domain code, and API (all three independently
agree: one `chunkText`/`embedding` per call, no chunk index, no batch)
but was nowhere explicitly documented as a *deliberate* scope boundary —
closed in this documentation turn (`03-Database-Architecture.md` §2.9's
own Phase 3 note, `08-Security.md` §5.4, and this section). A future
knowledge-document/email/meeting multi-chunk ingestion design will need
its own, separate identity scheme — this one is not it, and is not
meant to become it by extension.

### Content-hash semantics

`content_hash` is **not** part of row identity — `entity_profile_id` is
(enforced by the unique index above). `content_hash` is a sha256 hex
digest of `{entityType, entityId, chunkText}` (`node:crypto` only, no
dependency added), used exclusively to make a byte-identical redelivery
inside the update-in-place reconciliation flow a genuine no-op
(`status: "unchanged"`) rather than a wasted write. Independently
re-derived in a fresh script during acceptance audit: same input → same
hash, different content/entity/type → different hash, correct 64-hex
sha256 format.

### Write-back reconciliation, stale-write protection, concurrency model

`upsertEntityEmbedding` (`packages/brain/src/embeddings.ts`) reuses
`upsertEntityProfile`'s exact proven design verbatim: `SELECT ... FOR
UPDATE` serializes concurrent reconciliations for the same entity; a
freshness guard compares the caller-supplied `sourceVersionAt` against
both the *current* profile's `computed_at` and the existing embedding's
own `source_version_at`, rejecting anything strictly staler; a
first-insert race (`INSERT ... ON CONFLICT DO NOTHING` losing to a
concurrent winner) is resolved by looping back to the now-guaranteed-
to-succeed existing-row branch, bounded at 2 attempts by the identical
proof `upsertEntityProfile`'s own header comment already makes. Live-
verified during acceptance audit via a fresh standalone script (first
write creates; exact duplicate is a true no-op, same row; a stale
attempt is rejected; a newer write updates in place, same row, never a
second one; the final stored content correctly reflects only the
legitimate write) and a fresh 10-trial genuinely-simultaneous
concurrency run (zero uncaught errors, exactly one row every time).

**Accepted LOW limitation, disclosed here explicitly**: the freshness
comparison uses JS `Date` arithmetic; node-pg's default `timestamptz`
parser returns a `Date` object (millisecond precision), confirmed live
during acceptance audit (`typeof` / `instanceof Date` on a real
`computed_at` read). This mirrors — not introduces — an already-
accepted LOW limitation `upsertEntityProfile` itself carries for
`brain_entity_profiles.computed_at`: two recomputations landing within
the same millisecond resolve deterministically as last-write-wins under
the row lock, never as silent data loss (unlike Phase 2's own
pagination-cursor BLOCKER, which could silently skip a row entirely —
a materially different, more severe failure mode this does not share).

### API / service scope

`POST /api/v1/brain/embeddings`, API-key-authenticated,
`brain:embeddings:write` scope — confirmed live that `api_keys.scopes`
is an unconstrained `jsonb` array, so the new scope required zero
schema migration. `organization_id` always comes from the resolved
actor, never the request body. Vector validation (exactly 1536 finite
numbers) happens before persistence, at both the API boundary and
defensively inside the domain layer. PII/logging: independently
confirmed the structured request logger's `StructuredLogFields` type
has no field for a request body at all, so `chunkText`/`embedding`
cannot reach any log line structurally, not merely by convention — an
empirical regression test against the real logging-wrapped route
proves it. This is a machine/service credential surface, mirroring
Lead Enrichment's write-back precedent exactly — not a new end-user-
facing Brain API, UI, or RBAC permission.

### Dispatcher design and the unconfigured-webhook decision

Three new `EventConsumer`s, registered on the same nine CRM events the
Phase 2 projection consumers already drain, notify a future n8n
workflow via an ID-minimized payload, reusing `workflow_runs`' claim-
lease pattern under a distinct `brain_embedding_trigger_<entity>` key
family. **A defect found and fixed during implementation, before
acceptance**: the first draft threw when
`N8N_BRAIN_EMBEDDING_WEBHOOK_URL` was unconfigured, mirroring
`leadEnrichmentConsumer`'s own pattern literally — reproduced live under
full-monorepo test load as 7 test failures, including two *pre-existing,
already-accepted* Phase 2 tests. Root cause: because these three
consumers share the exact same high-frequency events as the Phase 2
projection consumers (unlike `visitor.identified`'s comparatively rare
volume), an indefinitely-retried failed delivery permanently occupies
the head of the shared `created_at`-ordered event queue, starving
Phase 2's own already-accepted delivery for the same and every later
event — a real regression risk to already-accepted behavior in any
deployment before the (not-yet-built) n8n workflow is configured. Fixed
by returning cleanly instead of throwing: the delivery becomes terminal
(an honest `workflow_runs` failure entry is still recorded for
observability), no indefinite retry occurs. **Accepted, explicitly
disclosed consequence**: an event whose delivery became terminal this
way is not automatically retried once an operator later configures the
webhook — recovery requires running the backfill discovery pass
(below), which has no automatic schedule wired up in this phase.

### Backfill design

`findProfilesNeedingEmbedding` (`packages/brain/src/backfill.ts`) is
identification-only — it generates no vector and makes no external
call. It pages `brain_entity_profiles` by `id` alone (a UUID, never a
timestamp), deliberately immune to the `timestamptz`-cursor microsecond-
precision class of bug Phase 2 had to fix once already, by construction
rather than by care — proven via a same-instant-timestamp collision
test. **Accepted LOW edge case, disclosed here**: unlike `created_at`-
based pagination, UUID order carries no "new rows always sort ahead of
the cursor" guarantee (UUIDs are random) — a profile created while a
backfill pass is in flight could, in principle, land behind that pass's
cursor and be missed by that one run. Not a permanent loss: the entity's
own live-event trigger path is independent of the backfill, and any
still-missing/stale profile is caught by a subsequent backfill run (the
"needs embedding" predicate is state-based — no current embedding, or
one older than the current `computed_at` — not tied to insertion
order).

### GDPR / hard-erasure behavior

No second erasure mechanism was added. A hard-erased contact's
...
(the new `entity_profile_id` FK is also `ON DELETE CASCADE`). Live-
proven end-to-end during acceptance audit: profile + embedding created
→ real `DELETE FROM contacts` → profile gone → embedding gone → a late
write-back arriving after erasure correctly rejected as
`entity_not_found`, nothing resurrected, zero embeddings remain for
that organization. Cross-tenant erasure isolation separately proven
(an unrelated organization's embedding survives, unaffected).

### Security / RLS change

`authenticated` gains `UPDATE` on `brain_embeddings` (confirmed live:
exactly `INSERT`/`SELECT`/`UPDATE`, still zero `DELETE`) — a deliberate,
necessary, and explicitly-disclosed extension over Phase 1's original
"written once, never edited in place" assumption (that migration's own
comment predates this phase's actual write-back design). Idempotent
"one current row per entity" reconciliation requires updating an
existing row in place; an insert-only "supersede with a new row"
alternative cannot coexist with the partial unique index without either
resurrecting the exact ordering-precision problem Phase 2 already fixed
or duplicating `brain_entity_profile_history`'s own already-solved job.
The one pre-existing test asserting the prior, narrower grant set
(`brain-rls.test.ts`) was updated to assert the new exact set with
`toEqual` (not weakened to a subset check) — still explicitly proving no
`DELETE` grant exists.

### Audit trail

One controlled implementation turn, delivering the full scope above
with two genuine defects found and fixed *during* implementation itself
(the CHECK-constraint-too-restrictive schema regression and the
unconfigured-webhook head-of-line-blocking risk, both described in full
above) — proven via full-monorepo re-verification after each fix, not
merely asserted. One strict read-only Final Implementation Acceptance
Audit followed, explicitly instructed not to trust the implementation
turn's own report: independently reproduced the schema/constraint
behavior via live SQL probes, the write-back reconciliation and
concurrency behavior via fresh standalone scripts (not the existing test
suite), the vector-validation and API-authorization behavior via fresh
test runs, and investigated (rather than accepting on faith) two
observed CI-equivalent flakes, root-causing both to genuine host-level
CPU oversubscription (load average measured at 11.12 on an 8-core
machine at the moment of one failure, with Postgres itself confirmed
idle) rather than any Phase-3 defect — **GO**, zero BLOCKER, zero HIGH,
three LOW/MEDIUM documentation-clarity findings (all closed by this
documentation turn) and two INFORMATIONAL notes (the manual-backfill
recovery requirement, already self-disclosed in code; the environmental
CI-equivalent flakiness).

### Final validation (Final Implementation Acceptance Audit, fresh)

Full monorepo test suite: **3216/3216** passed (a fresh, fully-reset
CI-equivalent `turbo run test --force` run, default concurrency — two
of three such runs during the audit were additionally clean with zero
deadlocks/chaos-leakage; the third's single failure was the
environmental flake above). `pnpm lint`/`typecheck`: **18/18** clean.
`pnpm build`: **1/1** successful, `/api/v1/brain/embeddings` confirmed
present in the build output. `pnpm audit --audit-level=high`: **"No
known vulnerabilities found."** Zero new dependencies —
`pnpm-lock.yaml` untouched by this phase entirely. `git diff --check`
clean throughout.

### Deviations from the M4.1 Phase 3 authorization

1. `content_hash`/`source_version_at` were originally designed as
   unconditionally `NOT NULL` with a matching CHECK requiring
   `entity_profile_id` whenever `source_type='entity_profile'` — this
   broke 17 pre-existing, already-accepted tests exercising Phase 1's
   own general multi-entity-chunk shape. Corrected to the conditional
   design described above before this phase's own implementation report
   was ever issued.
2. The dispatcher's unconfigured-webhook behavior was changed from
   "throw and retry indefinitely" (the literal `leadEnrichmentConsumer`
   mirror originally planned) to "return cleanly, terminal, backfill-
   recoverable" — a deliberate, disclosed, narrower design than the
   precedent it otherwise follows, for the head-of-line-blocking reason
   described above.
3. `authenticated`'s grant on `brain_embeddings` was widened to include
   `UPDATE`, correcting Phase 1's own original "never edited in place"
   assumption once the actual write-back design required it — not part
   of the original Detailed Design's explicit grant list.

**Milestone 4.1 Phase 3: PASS.** The embedding write-back foundation —
schema, domain reconciliation, scoped API, dispatcher triggers, and
backfill discovery — is live, adversarially verified via independent
live-Postgres reproduction and fresh standalone scripts (not merely
existing tests), and free of BLOCKER/HIGH findings. This repository
still generates no embedding and calls no AI/embedding provider —
semantic search, the retrieval client, knowledge-document/email/meeting
