...
**Implementation status (Milestone 4.1 Phase 5, implementation accepted — `docs/13-Technical-Design-Review.md` "Milestone 4.1 Phase 5"; Phase 4 — "Milestone 4.1 Phase 4"; Phase 3 — "Milestone 4.1 Phase 3"; Phase 2 — "Milestone 4.1 Phase 2"; Phase 1 — "Milestone 4.1 Phase 1"; "Milestone 4.1 Phase 5" is an internal implementation increment of this milestone, unrelated to `09-Development-Roadmap.md`'s own, separate "Phase 5 — Automations," which has not started):** everything in this document below this note is the Brain's target design; a meaningful slice is now built, most of it is not yet. **Shipped**: Phase 1's `pgvector` extension and six `brain_*` storage tables (schema, tenant-safe RLS/grants/retention) plus `execute_contact_erasure()` extended for Brain data; Phase 2's `packages/brain` domain package, CRM (contacts/companies/deals) domain-event emission via three `SECURITY DEFINER` functions, three dispatcher-driven projection consumers, and deterministic canonical-profile projection into `brain_entity_profiles`/`brain_entity_profile_history` — a **state-reconciliation model**: an event is a trigger to re-read the entity's *current* row and deterministically recompute its profile, using the source row's own `updated_at` for freshness, never a replay or accumulation of historical event payloads (unlike this section's own "incremental re-summarization" framing below, which describes the eventual AI-synthesized target, not what Phase 2 built). Soft-delete tombstones and GDPR hard-erasure cascade integration are both proven working for the projected entities. A resumable backfill/bootstrap CLI (`bootstrapBrainForOrganization`) projects pre-existing CRM data that predates Phase 2. Phase 3's **embedding write-back foundation**: `brain_embeddings` now has a real write path — `POST /api/v1/brain/embeddings` (a scoped, API-key-authenticated service endpoint, `brain:embeddings:write`) accepts an already-computed 1536-dimension vector from an external caller and persists it idempotently (one current embedding row per entity profile, concurrency-safe, stale-result-rejecting), plus dispatcher trigger consumers and a profiles-needing-embedding backfill discovery pass. **This repository still generates zero embeddings and calls zero AI/embedding providers — Phase 3 is a persistence/write-back layer only**, exactly mirroring how Phase 2 never called an AI model either; the actual n8n workflow that would compute a real vector and call this endpoint does not exist yet. See `docs/03-Database-Architecture.md` §2.9 for the real, current schema (which deviates from §4 below in real ways — a relational `brain_embedding_entity_refs` junction table instead of a jsonb `entity_refs` array, three-nullable composite-FK columns instead of a plain `entity_id`, no `version`/`summary`/`source_refs` columns; Phase 3's own `entity_profile_id`/`content_hash`/`source_version_at` are additive and opt-in, not part of this section's own original design at all). Phase 4's **tenant-safe semantic retrieval foundation**: `searchEntityProfilesByEmbedding` and `POST /api/v1/brain/search` (`brain:embeddings:read`) accept only an already-computed query vector and rank already-persisted embeddings via exact (non-ANN) pgvector cosine similarity, dual-layer tenant isolation, and SQL-side freshness exclusion — a narrower foundation than this document's own §5 `brain.semantic_search` target design below (no query text, no filters beyond `entityType`/`minSimilarity`, no persona-scoped post-filter, since no agent/persona layer exists yet). A zero-norm-vector defect (undefined cosine similarity, `minSimilarity`-threshold bypass) was found and classified HIGH during acceptance audit and corrected on both the query and stored-vector sides before final re-acceptance (`docs/13-Technical-Design-Review.md` "Milestone 4.1 Phase 4"). **This repository still generates zero embeddings, calls zero AI/embedding providers, and accepts zero query text anywhere.** Phase 5's **automated embedding-trigger recovery**: the operational Brain flow's live path (a CRM change → a domain event → a dispatcher trigger consumer → an n8n notification → an eventual embedding write-back) previously had a real, self-disclosed gap — a missed notification (an unconfigured-webhook window, a transient delivery failure, or an n8n webhook accepted but the embedding never actually written) had no automatic recovery. `GET /api/internal/brain-embedding-recovery`, on its own `*/15 * * * *` schedule, now re-discovers and re-notifies every such profile automatically via a state-based, cursor-free query (`findEmbeddingRecoveryCandidates`), making that flow genuinely self-healing rather than fire-once best-effort; a webhook 2xx response is deliberately never treated as proof the embedding materialized, only that n8n accepted the request. **This repository still generates zero embeddings and calls zero AI/embedding providers anywhere — Phase 5 only re-invokes the same not-yet-built n8n Brain Indexing workflow Phase 3 already targets, on a schedule, and adds no new AI/model-provider dependency.** **Not built**: any embedding *generation*, natural-language semantic search, the full `brain.semantic_search` agent tool/retrieval client (§5), AI-synthesized summarization, the continuous-learning loop (§6), ingestion for any non-CRM source (website visitors, tasks, manually-uploaded knowledge documents), `conversation_threads`/`chat_messages` (§3's own table below still incorrectly lists these as existing — they don't, see `docs/03-Database-Architecture.md` §2.4), and every new-raw-source table in §4 (`email_threads`/`email_messages`/`meetings`/`support_tickets`). At-least-once event delivery with idempotent processing is the real, disclosed contract throughout — never exactly-once. Rollout sequencing (§11) is unchanged: this remains Phase 4 work (this roadmap's own top-level "Phase 4 — AI Agents"), still in progress, not complete.
...
|---|---|---|---|
| CRM (contacts, companies, deals) | Exists — **deterministic projection delivered, Milestone 4.1 Phase 2** | Domain events (`contact.created`/`.updated`/`.deleted`, `company.*`, `deal.*`) trigger a re-read of current state and a deterministic canonical-profile recompute — not the AI-synthesized "incremental re-summarization" this row originally described, which remains not built (no embedding, no LLM involvement in projection) | Covered by existing `data_processing` consent |
| Activities | Exists in the CRM itself (`activities` table) — **not yet a Brain ingestion source**; no `activity.*` event type or projection consumer exists | Not built | Covered by existing `data_processing` consent |
...
| Conversations / chat | **Not built** — no `chat_messages`/`conversation_threads` table exists in this repository (`docs/03-Database-Architecture.md` §2.4); this row was inaccurate until corrected in Milestone 4.1 Phase 1's documentation pass | Embedded on message creation, once built | Covered by `data_processing` consent; portal-customer conversations gated additionally by portal terms acceptance |
...
| Revenue | Partially exists — `deals` exists; `revenue_events` does **not** (`docs/03-Database-Architecture.md` §2.6: no realized-revenue ledger has ever been built) | Same as CRM | Covered by `data_processing` consent |
...
**`brain.semantic_search`'s retrieval foundation is delivered, the tool itself is not (Milestone 4.1 Phase 4, `docs/13-Technical-Design-Review.md` "Milestone 4.1 Phase 4")**: `searchEntityProfilesByEmbedding`/`POST /api/v1/brain/search` perform the exact (non-ANN) pgvector cosine-similarity ranking this row describes, with the dual-layer `organization_id` defense-in-depth this row's own permission column anticipates — but accept only an already-computed `queryVector`, not natural-language `query` text (no query-text-to-vector step exists anywhere in this repository), support only `entityType`/`minSimilarity` filtering (not the broader `entity_refs`/`source_type`/date-range filter set this row describes), and have no persona/agent layer to apply the post-query permission re-check this row specifies, since no agent tool-execution layer exists yet. This is the ranking foundation this row's target design will be built on, not the row itself.
