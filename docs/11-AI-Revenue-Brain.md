...
**Implementation status (Milestone 4.1 Phase 1, delivered — `docs/13-Technical-Design-Review.md` "Milestone 4.1 Phase 1"):** everything in this document below this note is the Brain's target design, most of it not yet built. What's actually shipped so far is narrow: `pgvector` enabled, the six `brain_*` storage tables (schema, tenant-safe RLS/grants/retention), and `execute_contact_erasure()` extended for Brain data — see `docs/03-Database-Architecture.md` §2.9 for the real, current schema (which deviates from §4 below in real ways — a relational `brain_embedding_entity_refs` junction table instead of a jsonb `entity_refs` array, three-nullable composite-FK columns instead of a plain `entity_id`, no `version`/`summary`/`source_refs` columns). **Not built**: `packages/brain` itself, any ingestion (event-driven or otherwise), embedding generation, semantic search, the retrieval client/tools (§5), the continuous-learning loop (§6), `conversation_threads`/`chat_messages` (§3's own table below still incorrectly lists these as existing — they don't, see `docs/03-Database-Architecture.md` §2.4), and every new-raw-source table in §4 (`email_threads`/`email_messages`/`meetings`/`support_tickets`). Rollout sequencing (§11) is unchanged: this remains Phase 4 work, still in progress, not complete.
