...
| `visitor_sessions` | `id`, `organization_id` (added 3.1A, see note above), `visitor_id` (composite fk to `website_visitors`), `tracking_site_id` (composite fk to `tracking_sites`, not null), `started_at`, `ended_at`, `referrer`, `utm_source/medium/campaign`, `landing_page`, `device_type` | `tracking_site_id` is `NOT NULL` — every session originates from a resolved tracking credential. No ingestion write path exists yet (3.1C) — zero rows until then. |
