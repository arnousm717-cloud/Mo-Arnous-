...
**M1.6 implementation note**: all four tables below are implemented, RLS-isolated, and org_admin-only (`08-Security.md` §3.1's Decision F). Only `data_subject_requests.subject_type = 'user'` (a staff `public.users` row, added beyond the three originally documented here) has a real, executable erasure cascade — `contact`/`visitor`/`portal_user` remain schema-ready for Phase 2/6 but have no fulfillment logic yet, since none of those tables exist. The two Brain-related `consent_records.consent_type` values below are similarly schema-documented but not yet part of the actual `CHECK` constraint — they land with the Brain itself (Phase 4), matching this table's own original "new — see §2.9" framing, which was written before either existed.
