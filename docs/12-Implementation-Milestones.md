...
**Phase 3 — Website Intelligence**: 3.1 Tracking Script + Ingestion Endpoint (delivered — `docs/13-Technical-Design-Review.md` "Milestone 3.1 — Overall Closeout", closed) · 3.2 Visitor Identification (delivered — `docs/13-Technical-Design-Review.md` "Milestone 3.2 — Overall Closeout", closed) · 3.3 Lead Enrichment, AI Revenue OS side (delivered — `docs/13-Technical-Design-Review.md` "Milestone 3.3 — Overall Closeout", closed; the real n8n workflow JSON/provider selection remains a later n8n workflow-authoring sub-phase, not part of this closure) · 3.4 Rules-Based Lead Scoring (delivered — `docs/13-Technical-Design-Review.md` "Milestone 3.4 — Overall Closeout", closed; deterministic only, no AI/agent involvement — any future agent-adjusted scoring remains Phase 4 scope) · 3.5 Revenue Dashboard v1 (delivered — `docs/13-Technical-Design-Review.md` "Milestone 3.5 — Overall Closeout", closed; read-only deal/lead-score/visitor/recent-deal aggregates surfaced on `/dashboard`, calling `packages/crm`/`packages/intelligence` domain functions in-process — no new API route, no new permission, no chart library, no realized-revenue ledger)

**Phase 4 — AI Agents**: 4.1 Brain Foundation (schema, pgvector, existing-source ingestion) — **Phase 1 (Database + GDPR Foundation) delivered and accepted** (`docs/13-Technical-Design-Review.md` "Milestone 4.1 Phase 1"): pgvector, the six `brain_*` tables, tenant-safe RLS/grants/retention, and targeted-capture GDPR erasure integration; **Phase 2 (CRM Domain Events + Deterministic Projections) delivered and accepted** (`docs/13-Technical-Design-Review.md` "Milestone 4.1 Phase 2"): `packages/brain`, nine CRM event types with `SECURITY DEFINER` emission, three dispatcher-driven projection consumers, deterministic state-reconciliation profiles, soft-delete tombstones, GDPR hard-erasure cascade integration, and a resumable backfill CLI — two genuine defects (a first-insert projection-concurrency race, a pagination-cursor microsecond-precision bug) were found across two independent NO-GO audit rounds and fixed before final acceptance; **Phase 3 (Embedding Write-Back Foundation) implementation accepted** (`docs/13-Technical-Design-Review.md` "Milestone 4.1 Phase 3"): an additive, opt-in single-row-per-`brain_entity_profiles` identity on `brain_embeddings` (`entity_profile_id`/`content_hash`/`source_version_at`, a conditional integrity constraint, a partial unique index), a concurrency-safe update-in-place write-back (`upsertEntityEmbedding`), the `POST /api/v1/brain/embeddings` scoped service endpoint (`brain:embeddings:write`), dispatcher trigger consumers, and a profiles-needing-embedding backfill discovery pass — **this repository still calls no embedding provider and generates no embedding itself**, it only persists an already-computed vector supplied by an external caller; **Phase 4 (Tenant-Safe Semantic Retrieval Foundation) implementation accepted** (`docs/13-Technical-Design-Review.md` "Milestone 4.1 Phase 4"): `searchEntityProfilesByEmbedding` (`packages/brain/src/search.ts`) and the scoped `POST /api/v1/brain/search` service endpoint (`brain:embeddings:read`) — exact (non-ANN) pgvector cosine-similarity retrieval over a caller-supplied, already-computed query vector only, dual-layer tenant isolation (RLS + explicit `organization_id` SQL predicate), SQL-side freshness exclusion, deterministic UUID tie-break ordering, and a minimized 5-field result shape; a zero-norm-vector defect (undefined cosine similarity silently serializing as `null` and defeating `minSimilarity` filtering at any threshold) was found live during the Final Implementation Acceptance Audit, classified HIGH, and corrected — both query-side rejection and stored-side `vector_norm(...) > 0` exclusion — before final re-acceptance; **this repository still calls no embedding provider, accepts no query text, and performs no natural-language semantic search**; **Milestone 4.1 Phase 5 (Automated Embedding-Trigger Recovery) implementation accepted** (`docs/13-Technical-Design-Review.md` "Milestone 4.1 Phase 5" — an internal Milestone-4.1 implementation increment, not to be confused with this document's own separate "Phase 5 — Automations" below, which has not started): gives Phase 3's own `findProfilesNeedingEmbedding` discovery pass a real, scheduled runtime caller (`GET /api/internal/brain-embedding-recovery`, `CRON_SECRET`, `*/15 * * * *`), closing the gap Phase 3 itself disclosed. A HIGH defect (a successful webhook 2xx response initially, incorrectly treated as permanent proof the embedding had materialized, permanently blocking recovery for that profile) was found live during the Final Implementation Acceptance Audit and corrected — a dedicated, cooldown-based claim function (`claimEmbeddingRecoveryAttempt`), no migration required — before final re-acceptance; **this repository still calls no embedding provider and holds no provider credential**; **remaining 4.1 scope (non-CRM source ingestion, actual embedding generation/provider integration, query-text-to-vector, RAG/agent retrieval tooling) not started — Milestone 4.1 as a whole remains IN PROGRESS, not complete** · 4.2 Queued Agent-Execution Worker · 4.3 Orchestrator + Model Router + Tool Layer + Approval Gate · 4.4 Research Agent · 4.5 Sales Agent · 4.6 Scoring Agent · 4.7 Marketing Agent · 4.8 Support/Chat Agent · 4.9 AISurfaceCard (3 variants) + Agent Monitoring
