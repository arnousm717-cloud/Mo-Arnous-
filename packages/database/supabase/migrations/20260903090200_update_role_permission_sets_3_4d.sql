-- Updates roles.permission_set to reflect Milestone 3.4D's 2 new
-- PermissionKeys (scoring-rules:read, scoring-rules:write). Same
...
-- 2.2C 20260814110000, 2.3C 20260817090300, 2.4B 20260819100400, 3.2B
-- 20260821090400) -- packages/auth/src/permissions.ts's PERMISSION_MATRIX
-- remains the sole source of truth; packages/auth/tests/
-- permission-set-sync.test.ts is what keeps this column from silently
-- drifting out of sync with it.
--
-- Only org_admin gains these two keys (same reasoning as
-- tracking:manage-identity-keys' own precedent, applied identically): no
-- agency-scoped role has any access to scoring-rule configuration, so
-- agency_owner/agency_admin/org_member/org_viewer/portal_customer are
-- left untouched.
--
